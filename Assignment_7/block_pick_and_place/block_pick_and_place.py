...
from transform_utils import (
    homogeneous_transform_to_pos_quat,
    pos_quat_to_homogeneous_transform,
)
...
class PandaSim:
    ...
    def set_joint_state(self, state):
        ...
        for i in range(7):
            p.setJointMotorControl2(
                self.panda, i, p.POSITION_CONTROL, state[i], force=5 * 240.0
            )
        self.step()
    ...
    def grasp(self):
        ...
        for i in [9, 10]:
            p.setJointMotorControl2(self.panda, i, p.POSITION_CONTROL, 0.003, force=10)
    ...
    def ungrasp(self):
        ...
        for i in [9, 10]:
            p.setJointMotorControl2(self.panda, i, p.POSITION_CONTROL, 0.04, force=10)
    ...
    def fk_and_jacobian(self, joint_state):
        ...
        return (
            gripper_pose,
            np.vstack([np.array(linear_jacobian), np.array(angular_jacobian)])[:, :7],
        )
